...
class TechnicalIndicatorCalculator:
    ...
    def calculate_all_indicators(self, data):
        ...
        fastk = talib.STOCHF(high, low, close, fastk_period=9)
            
        # 確保數組長度與索引匹配
        if len(fastk) == len(data.index):
            indicators['RSV'] = fastk
        else:
            indicators['RSV'] = np.nan
                
        if len(slowk) == len(data.index):
            indicators['K_Value'] = slowk
            indicators['D_Value'] = slowd
            indicators['J_Value'] = 3 * slowk - 2 * slowd
        else:
            indicators['K_Value'] = np.nan
            indicators['D_Value'] = np.nan
            indicators['J_Value'] = np.nan
